...
export async function GET(req: Request) {
  ...
  try {
    const plans = await prisma.note.findMany({
      ...
      select: { id: true, title: true, content: true, tags: true, pinned: true, projectId: true, createdAt: true, updatedAt: true },
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (action === "approve") {
      if (!planNoteId) return NextResponse.json({ error: "planNoteId required" }, { status: 400 })
      ...
      return NextResponse.json({ success: true, action: "approved", planNoteId, approvedBy: userName, message: "Plan approved. The agent will proceed with execution." })
    }
    ...
    if (action === "reject") {
      ...
      await prisma.note.update({
        ...
        data: {
          ...
          tags:    ["agent-plan", "rejected"], pinned: false,
        },
      })
      return NextResponse.json({ success: true, action: "rejected", planNoteId, rejectedBy: userName, feedback, message: "Plan rejected." })
    }
